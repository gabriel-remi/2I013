	...
	public void update() {
		...
		if(this.distance(this.cur_pos.x, this.cur_pos.y, player.cur_pos.x, player.cur_pos.y) < 10) {
			...
			for(Agent agent : Ecosystem.agents) {
				if(agent instanceof MiniRobot && this.distance(this.player.cur_pos.x, this.player.cur_pos.y, agent.cur_pos.x, agent.cur_pos.y) < 5) {
					agent.life--;
				}
			}
			
			for(int x = this.moduloWidth(this.player.cur_pos.x - 5); x < this.moduloWidth(this.player.cur_pos.x  +5); x++) {
				for(int y = this.moduloWidth(this.player.cur_pos.y - 5); y < this.moduloWidth(this.player.cur_pos.y  +5); y++) {
